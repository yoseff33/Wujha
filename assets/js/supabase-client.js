...
export const products = {
  ...
  get: id => supabase.from('products').select('*,seller:users!seller_id(id,name),region:regions(name_ar),city:cities(name_ar),district:districts(name_ar)').eq('id',id).single(),
  ...
  isFavorite: (userId,productId) => supabase.from('product_favorites').select('product_id').eq('user_id',userId).eq('product_id',productId).maybeSingle()
};

export const locations = {
  regions: () => supabase.from('regions').select('*').order('name_ar'),
  cities: regionId => supabase.from('cities').select('*').eq('region_id',regionId).order('name_ar'),
  districts: cityId => supabase.from('districts').select('*').eq('city_id',cityId).order('name_ar')
};
